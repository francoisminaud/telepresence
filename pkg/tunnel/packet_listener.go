...
func NewUDPListener(conn *net.UDPConn, targetIP net.IP, targetPort uint16, creator func(context.Context, ConnID) (Stream, error)) Endpoint {
	state := notConnected
	if conn != nil {
		state = connecting
	}
...
func (h *udpListener) connToStreamLoop(ctx context.Context) {
	...
	for atomic.LoadInt32(&h.connected) == connected {
		...
		case rr, ok := <-ch:
			...
			h.ResetIdle()
			id := NewConnID(IPProto(rr.Addr.Network()), rr.Addr.IP, h.targetIP, uint16(rr.Addr.Port), h.targetPort)
...
type pkgStream struct {
	TimedHandler
	*udpListener
	stream Stream
}

func (h *pkgStream) handleControl(ctx context.Context, cm Message) {
	switch cm.Code() {
...
func (p *pkgStream) Stop(ctx context.Context) {
	_ = p.stream.CloseSend(ctx)
}

func (p *pkgStream) Start(ctx context.Context) {
